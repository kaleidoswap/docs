...
<Note>
Since SDK 0.1.14 (RLN v0.8.0), `createRgbInvoice`/`create_rgb_invoice` requires `expiration_timestamp` and `transport_endpoints` in the request body, and `sendRgb`/`send_rgb` requires `expiration_timestamp`. See [RLN API Compatibility](/sdk/rln-api-compatibility).
</Note>
