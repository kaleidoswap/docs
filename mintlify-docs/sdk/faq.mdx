...
<AccordionGroup>
  ...
  <Accordion title="Which networks can I build against?" icon="network-wired">
    ...
    Pass this as `baseUrl` / `base_url`; the SDK appends `/api/v1` itself. Full list in [Available Environments](/sdk/installation#available-environments).
  </Accordion>
  ...
  <Accordion title="How should I handle errors and retries?" icon="triangle-exclamation">
    ...
    See [Troubleshooting](/sdk/troubleshooting#error-reference) for the full hierarchy and the retry-with-backoff pattern, and [Best Practices](/sdk/best-practices) for how it fits a production client.
  </Accordion>
</AccordionGroup>

## Get Help

Check the [Troubleshooting](/sdk/troubleshooting) for errors rather than questions, and [Additional Resources](/sdk/additional-resources) for upstream documentation and links.
