...
<AccordionGroup>
  <Accordion title="Cannot connect to node" icon="link-slash">
    ...
    **Solutions:**
    ...
    6. Try disconnecting and reconnecting in Settings > RGB
  </Accordion>
  ...
  <Accordion title="Provider not detected by a DApp" icon="eye-slash">
    ...
    **Solutions:**
    ...
    4. Confirm the DApp uses a standard provider -- the KaleidoSwap Extension injects `window.webln` (WebLN), `window.webbtc`, `window.nostr` (NIP-07), and `window.bitcoin`
    5. Some apps wait for a ready event -- the extension dispatches `webln:ready`, `webbtc:ready`, `nostr:ready`, and `bitcoin:ready`
  </Accordion>
</AccordionGroup>
...
Check the [FAQ](/extensions/kaleidoswap-extension/faq) for questions rather than errors, and [Additional Resources](/extensions/kaleidoswap-extension/additional-resources) for upstream documentation and links.
