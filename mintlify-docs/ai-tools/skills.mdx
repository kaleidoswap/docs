...
| `portfolio-loop` | Scheduled portfolio rebalancer: measures drift against targets, executes the minimum atomic swap, folds in DCA |
| `node-loop` | Scheduled node-health monitor: channel liquidity audit, stuck RGB transfer flush, LSPS1 auto-purchase below threshold |
| `bitrefill` | Gift cards, mobile top-ups, and eSIMs through Bitrefill, payable over Lightning |

`portfolio-loop`, `node-loop`, and `bitrefill` are the skills enabled by default in `agent.config.json`.
