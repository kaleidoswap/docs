...
<AccordionGroup>
  ...
  <Accordion title="Server starts then exits immediately" icon="circle-xmark">
    ...
    **Cause**: A required environment variable is missing. The standalone `wdk-wallet-spark-mcp` refuses to start without `WDK_SPARK_SEED`. `kaleido-mcp` is more forgiving: without `WDK_SEED` it still boots, logs a warning, and disables the Spark tools.

    **Solutions**:
    1. Confirm the seed variable is set in the `env` block of the host config, not in your shell (the host does not inherit your shell environment).
...
<AccordionGroup>
  ...
  <Accordion title="KaleidoCLI not found" icon="terminal">
    ...
    **Solutions**:
    ...
    3. Install it per [CLI Installation](/cli/installation) if it is missing.
  </Accordion>
...
Check the [FAQ](/ai-tools/faq) for questions rather than errors, and [Additional Resources](/ai-tools/additional-resources) for upstream documentation and links.
