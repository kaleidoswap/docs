---
title: Installation
description: Install or load the Rate browser extension
---

## Browser Requirements

Rate is built as a Manifest V3 web extension.

| Browser | Supported |
|---------|-----------|
| **Google Chrome** | Yes, Chromium extension support |
| **Brave** | Yes, Chromium extension support |
| **Edge** | Yes, Chromium extension support |
| **Opera** | Yes, Chromium extension support |
| **Firefox** | Supported by the manifest gecko configuration |
| **Safari** | Not yet |

## Installation

### From a Browser Store

When a browser-store build is available, install it from the Rate listing for your browser and pin the extension icon in the toolbar.

<Steps>
  <Step title="Open the Store Listing">
    Open the Rate wallet listing for your browser.
  </Step>
  <Step title="Install the Extension">
    Confirm the browser installation prompt.
  </Step>
  <Step title="Pin the Extension">
    Pin Rate from the extensions menu so the wallet is available from the toolbar.
  </Step>
</Steps>

### Manual Installation (Developer Mode)

Use manual installation when testing a pre-release build or building from source.

<Steps>
  <Step title="Build the Extension">
    First, download or clone the Rate source code, then open a terminal in the project root directory. If `pnpm` is not installed yet, install it first, then install dependencies and build:

    ```bash
    git clone <repository-url>
    cd <repository-directory>
    npm install -g pnpm
    pnpm install
    pnpm run build
    ```
  </Step>

  <Step title="Open Extensions Page">
    Navigate to `chrome://extensions` in your browser.
  </Step>

  <Step title="Enable Developer Mode">
    Toggle **Developer mode** in the top-right corner.
  </Step>

  <Step title="Load Unpacked">
    Click **Load unpacked** and select the `dist` folder from the build output.
  </Step>

  <Step title="Pin the Extension">
    Pin the Rate icon in your toolbar for easy access.
  </Step>
</Steps>

## First Launch

After installation, clicking the Rate icon in your toolbar opens the extension popup. On first launch, you are guided through the **onboarding flow**:

<img src="/images/rate/01-welcome.png" alt="Rate first launch" style={{ maxWidth: "360px", width: "100%", height: "auto" }} />

1. **Welcome screen** - Create a new wallet or import an existing phrase
2. **Password setup** - Create a secure password to protect the local wallet
3. **Recovery phrase backup** - Save the phrase that controls Spark, Arkade, and Nostr identity
4. **Optional RLN setup** - Connect an RGB Lightning Node, or skip and connect it later
5. **Ready** - Continue to the wallet dashboard

See [Getting Started](/extensions/rate/getting-started) for more information.

## Permissions

Rate requests the following Chrome extension permissions:

| Permission | Purpose |
|-----------|---------|
| **Storage** | Store encrypted wallet data, preferences, and account configuration locally |
| **Active Tab** | Support browser-page wallet interactions |
| **Scripting** | Inject DApp bridge scripts |
| **Tabs** | Coordinate approval and confirmation windows |
| **Alarms** | Run scheduled wallet/session tasks |
| **Notifications** | Notify the user about wallet events where supported |

<Note>
Rate stores the encrypted mnemonic in `chrome.storage.local`. Network calls are made only for the accounts and services you connect, such as Spark, Arkade, an optional RGB Lightning Node, price data, or swap venues.
</Note>

## Updating

Store-installed builds update through the browser's extension update system. For manual installations, pull the latest code, rebuild, and reload the unpacked extension in your browser.

## Uninstalling

To remove the Rate extension proceed with the instructions provided by your browser.

<Warning>
Uninstalling the extension removes all locally stored wallet data. Make sure you have backed up your Nostr keys and any important data before uninstalling.
</Warning>
