...
<AccordionGroup>
  <Accordion title="TypeScript 还是 Python —— 我该用哪个？" icon="code">
    看你的技术栈。两者都是独立实现，类型都由同一份 [OpenAPI 规范](https://github.com/kaleidoswap/specs)生成，因此 API 结构、子客户端和错误层级在两者之间几乎完全一致 —— 少数差异（速率限制处理、节点缺失错误）已在[故障排查](/cn/sdk/troubleshooting#error-reference)中说明。本文档中的每个示例都同时给出两种语言。
  ...
  <Accordion title="我可以在哪些网络上开发？" icon="network-wired">
    ...
    把它作为 `baseUrl` / `base_url` 传入即可；`/api/v1` 由 SDK 自动追加。完整列表见[可用环境](/cn/sdk/installation#available-environments)。
  </Accordion>
  ...
  <Accordion title="我应该如何处理错误和重试？" icon="triangle-exclamation">
    ...
    完整的错误层级和带退避的重试模式见[故障排查](/cn/sdk/troubleshooting#error-reference)，如何将其融入生产级客户端见[最佳实践](/cn/sdk/best-practices)。
  </Accordion>
</AccordionGroup>

## 获取帮助

如果遇到的是报错而不是疑问，请查阅[故障排查](/cn/sdk/troubleshooting)；上游文档与相关链接见[更多资源](/cn/sdk/additional-resources)。
