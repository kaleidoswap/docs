...
| Step | Endpoint |
|------|----------|
| Request quotes | WebSocket `wss://api.signet.kaleidoswap.com/api/v1/market/ws/{client_id}` |
| Lock rate | `POST /api/v1/swaps/init` |
| Execute swap | `POST /api/v1/swaps/execute` |
| Check status | `POST /api/v1/swaps/atomic/status` (requires `access_token` from init) |

### WebSocket Environments

| Environment | URL |
|-------------|-----|
| **Regtest** | `wss://api.regtest.kaleidoswap.com/api/v1/market/ws/{client_id}` |
