...
|------|-------------|-------------|
| `skill` _(default)_ | `SKILL.md` files load as the system prompt, and the agent calls a single `run_kaleido_command` tool that shells out to KaleidoCLI. Nanobot manages all MCP connections directly | Lightweight, no direct MCP connection from Node.js |
