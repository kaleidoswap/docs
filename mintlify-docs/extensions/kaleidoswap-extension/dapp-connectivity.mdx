...
The KaleidoSwap Extension injects APIs into browser pages so DApps can interact with wallet capabilities after user approval. This supports RGB WebLN-style requests, wallet approval flows, and Nostr signing.
...
The KaleidoSwap Extension derives a Nostr identity from the wallet recovery phrase and exposes signing support for compatible Nostr clients.

### How It Works

1. The KaleidoSwap Extension provides a Nostr-compatible signing API
2. Nostr clients call signing methods
3. The KaleidoSwap Extension prompts you to approve the signing request
