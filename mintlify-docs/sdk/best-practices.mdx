...
description: 'Development guidelines for building with the KaleidoSwap SDK'
---
...
```python Python
from kaleidoswap_sdk import to_smallest_units, PairQuoteRequest, AssetInput
...
quote = client.maker.get_quote(PairQuoteRequest(
    from_asset=AssetInput(asset_id="BTC", layer="BTC_LN", amount=raw_amount),
...
def get_pairs():
    ...
    if _cached_pairs is None or now - _cache_time > 60:
        _cached_pairs = client.maker.list_pairs()
        _cache_time = now
