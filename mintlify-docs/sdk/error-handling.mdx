...
description: "Handle KaleidoSDK errors with the exception hierarchy, covering error types, retry patterns, and debugging tips for reliable integrations"
---

## Error Hierarchy

Both SDKs share a nearly identical error class hierarchy. All errors extend from `KaleidoError`:
...
One divergence to be aware of: in TypeScript `RateLimitError` extends `APIError`, while in Python it extends `KaleidoError` directly — a Python `except APIError` handler will not catch rate-limit errors.
...
Raised when rate limit is exceeded (HTTP 429). Extends `APIError` in TypeScript; in Python it extends `KaleidoError` directly.
...
Raised by the Python SDK when an RLN node operation is attempted without a configured `node_url`. TypeScript instead throws `ConfigError` ("Node API not configured. Provide \"nodeUrl\" when creating the client.") when a `client.rln.*` method is called without `nodeUrl`.
...
// Properties: requiredAmount: number, availableAmount: number, asset?: string
