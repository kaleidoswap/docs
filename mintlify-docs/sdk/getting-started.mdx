...
You can configure the SDK using environment variables. Copy this template to a `.env` file in your project root:

```bash .env.example
# Required: KaleidoSwap API base URL
# Options: https://api.regtest.kaleidoswap.com | https://api.signet.kaleidoswap.com
KALEIDO_API_URL=https://api.regtest.kaleidoswap.com

# Optional: URL of your RGB Lightning Node (required for wallet/channel operations)
KALEIDO_NODE_URL=http://localhost:3001

# Optional: API key for authenticated endpoints (not currently required)
KALEIDO_API_KEY=
```

Then load variables from your environment:
...
if client.has_node():
    ...
    invoice = client.rln.create_ln_invoice({...})
```
</CodeGroup>
