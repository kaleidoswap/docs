...
## Maker API Connection

Rate can optionally connect to the **Kaleidoswap maker API** to enable swap functionality:

- **Default URL**: `https://api.regtest.kaleidoswap.com`
- Configurable in **Settings > Networks**

The maker connection is **not required** for core wallet features — send, receive, and asset management work without it. However, **the swap feature is disabled without a maker API connection**. To use swaps, ensure a valid maker API URL is configured in Settings.
