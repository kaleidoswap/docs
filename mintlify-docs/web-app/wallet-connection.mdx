...
The KaleidoSwap Extension is a browser wallet purpose-built for Bitcoin Layer 2 networks:

- **Multi-Protocol** - Supports Spark, Arkade, and optional RGB Lightning Node accounts
- **Built-in Swaps** - Swap assets directly within the extension via KaleidoSwap maker pairs and supported Spark/Flashnet venues
- **WebLN Provider** - Exposes `window.rgbwebln` for seamless DApp integration
- **Nostr Identity** - Recovery-phrase-derived Nostr key management in extension settings
- **Pay with Wallet** - On the order page, pay deposit invoices directly from the KaleidoSwap Extension without leaving the browser

**Connection Flow:**
1. Select KaleidoSwap Extension from the wallet list
2. The KaleidoSwap Extension popup appears requesting permission
3. Approve the connection
4. Your node address is linked and you can pay invoices directly from the order page

<Tip>
With the KaleidoSwap Extension connected, you can use the **Pay with Wallet** button on the order page to pay your deposit invoice without switching apps.
</Tip>
